...
class AffectImage(ui.ExpandedImageBox):
	...
	if app.ENABLE_SET_ITEM:
		def SetToolTipText(self, text, x=0, y=-19, adjust_line_height=False, line_height_distance=20):
			if not self.toolTipText:
				...
				if osfInfo.AFFECT_DISPLAY_IN_THINBOARD:
					if len(self.toolTipText.childrenList):
						self.toolTipText.childrenList[0].SetText(text)
		...
		def SetToolTipText(self, text, x = 0, y = -19):
			if not self.toolTipText:
				if osfInfo.AFFECT_DISPLAY_IN_THINBOARD:
					...
					textLine.ClearToolTip()
					textLine.AutoAppendTextLine(text)
					textLine.AlignTextLineHorizonalCenter()
					textLine.Hide()
				else:
					textLine = ui.TextLine()
					textLine.SetParent(self)
	...
	if app.ENABLE_SET_ITEM:
		def UpdateSetItemDescription(self, affect_type = chr.NEW_AFFECT_SET_ITEM):
			...
			if not self.tooltipItem:
				...
				toolTip = "|cffffb96d" + self.description + "|cff89b88d"
				toolTip += "\\n"
				#toolTip += "|cff89b88d"
				...
				if type(self.affect_dict) is dict:
					if len(self.affect_dict) > 0:
						for k in self.affect_dict.keys():
							toolTip += "|cff89b88d" + self.tooltipItem.GetAffectString(k, self.affect_dict[k]) + "|cff89b88d"
							toolTip += "\\n"
							#toolTip += "|cffe57875"
					if len(self.affect_dict) < 3:
						toolTip += ""
						toolTip += "Neith2"

			self.SetToolTipText(toolTip, -20, 40, True, 15)
	...
	def OnMouseOverIn(self):
		if self.toolTipText:
			if self.affect == chr.NEW_AFFECT_SET_ITEM or self.affect >= chr.NEW_AFFECT_SET_ITEM_SET_VALUE_1 and self.affect <= chr.NEW_AFFECT_SET_ITEM_SET_VALUE_5:
				self.toolTipText.AppendSpace(35)
			self.toolTipText.Show()
...
class AffectShower(ui.Window):
	...
	if app.ENABLE_CONQUEROR_LEVEL:
		...
		AFFECT_DATA_DICT[chr.NEW_AFFECT_SUNGMA_HP] = (localeInfo.TOOLTIP_AFFECT_SUNGMA_HP_BUFF, "d:/ymir work/ui/skill/common/affect/sungma_hp_buff.sub")
		AFFECT_DATA_DICT[chr.NEW_AFFECT_SUNGMA_IMMUNE] = (localeInfo.TOOLTIP_AFFECT_SUNGMA_IMMUNE_BUFF, "d:/ymir work/ui/skill/common/affect/sungma_immune_buff.sub")
		AFFECT_DATA_DICT[chr.NEW_AFFECT_SUNGMA_MOVE] = (localeInfo.TOOLTIP_AFFECT_SUNGMA_MOVE_BUFF, "d:/ymir work/ui/skill/common/affect/sungma_move_buff.sub")
		AFFECT_DATA_DICT[chr.NEW_AFFECT_SUNGMA_STR] = (localeInfo.TOOLTIP_AFFECT_SUNGMA_STR_BUFF, "d:/ymir work/ui/skill/common/affect/sungma_str_buff.sub")
				...
				for image in self.affectImageDict.values():
					...
					if app.ENABLE_SET_ITEM:
						if image.GetAffect() == chr.NEW_AFFECT_SET_ITEM or image.GetAffect() >= chr.NEW_AFFECT_SET_ITEM_SET_VALUE_1 and image.GetAffect() <= chr.NEW_AFFECT_SET_ITEM_SET_VALUE_5:
							image.UpdateSetItemDescription(image.GetAffect())
							continue
